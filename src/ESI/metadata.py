...
try:
    ...
    # log
    METADATA_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), "metadata.json"))
...
class ESIMetadata(object):
    ...
    def _parse_parameters(self, body: dict) -> ESIParams:
        ...
        for param in parameters:
            ...
            params.append(Param(param['name'], param["in"], param.get("required", False), param.get("type", ""), ))
    ...
    def _parse_security(self, body: dict) -> List[str]:
        ...
        if (len(scope_) > 1):
            # log warning
            raise ValueError(f"API request with multiple scopes is not supported.\
    ...
    def _load_metadata(self) -> None:
        ...
        if not os.path.exists(METADATA_PATH) or os.stat(METADATA_PATH).st_size == 0:
            r = requests.get("https://esi.evetech.net/latest/swagger.json?datasource=tranquility")
            r.raise_for_status()
            metadata = r.json()
        ...
        params = [Param(f"{v['name']}", v['in'], v.get('required', False), v['type'], v.get('default', None)) for v in metadata['parameters'].values()]
        self._metaParams = ESIParams(params) 

    # Helpful functions

    def print_names(self, _in: Optional[str] = None, required: Optional[bool] = None, default: Optional[bool] = None) -> None:
        ...
        for key in self.paths.keys():
            try:
                for param in self.__getitem__(key).parameters:
                    if (_in and param._in != _in) or param.name in ins or (required is not None and param.required != required) or (default and not param.default):
                        continue
